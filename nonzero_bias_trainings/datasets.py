...
class MNIST:
    def __init__(self, batch_size, threads, aug='none', train_count=None, num_classes=2, seed=10, classes=None):
        mean, std = self._get_statistics()
        torch.manual_seed(seed)
        if aug == "none":
            train_transform = transforms.Compose([
        ...
        if classes is not None:
            selected_classes = torch.tensor(classes)
        else:
            selected_classes = torch.arange(10)[torch.randperm(10)][:num_classes]
        for i, cur_class in enumerate(selected_classes):
