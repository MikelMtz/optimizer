...
class MLPModels(nn.Module): 
    def __init__(self, input_dim, output_dim, layers, hidden_units, model_count, device):
        ...
        self.bias = nn.Parameter(torch.zeros((1, model_count, output_dim), device=device))
    ...
    @torch.no_grad()
    def reinitialize_small(self):
        """Uniform [-0.2, 0.2] initialization."""
        for matrix in self.weights:
            torch.nn.init.uniform_(matrix.data, a=-0.2, b=0.2)
        self.bias.data.zero_()

    @torch.no_grad()
    def reinitialize_kaiming_uniform(self):
    ...
    @torch.no_grad()
    def reinitialize_kaiming_normal(self):
    ...
    def forward(self, x):
        # the forward takes in x shaped [# of examples, input_dim] (or [N, C, H, W])
        # outputs [# of examples, model_count, logit_count]
        x = x.reshape(x.size(0), -1) #NEW
        x = x[:, None, None]
    ...
    def set_lipschitz_ref_data(self, ref_data):
        """Store reference data (train ∪ test) for Lipschitz normalization."""
        self._lip_ref_data = ref_data

    @torch.no_grad()
    def compute_lipschitz_constant(self, ref_data, batch_size=8):
        """Compute max_z ||J_f(z)||_F over z in ref_data, per model.
        Analytically computes the Jacobian for ReLU MLPs.
        Returns shape (model_count,)."""
        max_lip = torch.zeros(self.model_count, device=self.weights[0].device)
        for st in range(0, len(ref_data), batch_size):
            x_batch = ref_data[st:st+batch_size]
            x_batch = x_batch.reshape(x_batch.size(0), -1)  # flatten to (B, d_in)
            # Forward pass to collect ReLU masks
            h = x_batch[:, None, None]  # (B, 1, 1, d_in)
            masks = []
            for matrix in self.weights[:-1]:
                h = h @ matrix  # (B, M, 1, h_l)
                masks.append((h > 0).float())
                h = h * masks[-1]
            # Jacobian: J = (W0 * D0) @ (W1 * D1) @ ... @ W_L
            J = self.weights[0] * masks[0]  # (B, M, d_in, h_0)
            for i in range(1, len(self.weights) - 1):
                J = J @ (self.weights[i] * masks[i])
            J = J @ self.weights[-1]  # (B, M, d_in, d_out)
            grad_norm = (J ** 2).sum(dim=(2, 3)).sqrt()  # (B, M)
            max_lip = torch.max(max_lip, grad_norm.max(dim=0).values)
        return max_lip

    def forward_normalize_lipschitz(self, x):
        """Normalize output by Lipschitz constant estimated on reference data."""
        logits = self.forward(x)
        lip_const = self.compute_lipschitz_constant(self._lip_ref_data)
        lip_const = lip_const.clamp(min=1e-8)
        return logits / lip_const[None, :, None]

    def forward_normalize_frobenius(self, x):
        """Original Frobenius-norm-based normalization."""
        cum_norm = 1
        for weight in self.weights:
            cur_norm = weight.norm(dim=(2,3), keepdim=True)
            cum_norm *= cur_norm
    ...
    def set_norm_mode(self, mode):
        """Set normalization mode: 'frobenius' or 'lipschitz'."""
        self._norm_mode = mode

    def forward_normalize(self, x):
        if getattr(self, '_norm_mode', 'frobenius') == 'lipschitz' and \
                hasattr(self, '_lip_ref_data') and self._lip_ref_data is not None:
            return self.forward_normalize_lipschitz(x)
        return self.forward_normalize_frobenius(x)
...
class LeNetModels(nn.Module):
    def __init__(self, output_dim, width_factor, model_count, dataset, feature_dim=None, depth=4):
        super(LeNetModels, self).__init__()
        self.model_count = model_count
        self.output_dim = output_dim
        self.width_factor = width_factor
        self.dataset = dataset
        self.depth = depth  # 4=2c-3f, 3=2c-2f, 2=2c-1f, 1=1c-1f
        if feature_dim is None:
            self.feature_dim = int(84 * width_factor)
        else:
            self.feature_dim = feature_dim

        # Spatial flat sizes after each conv+pool stage
        if dataset == "cifar10":
            _c_in = 3
            _after_1conv_flat = int(6 * width_factor) * 14 * 14  # (32-5+1)/2=14
            _after_2conv_flat = int(16 * width_factor) * 5 * 5   # (14-5+1)/2=5
        else:  # mnist, kink: 28×28 input
            _c_in = 1
            _after_1conv_flat = int(6 * width_factor) * 12 * 12  # (28-5+1)/2=12
            _after_2conv_flat = int(16 * width_factor) * 4 * 4   # (12-5+1)/2=4

        # conv1: always present
        self.conv1 = nn.Conv2d(_c_in * model_count,
                               int(6 * width_factor) * model_count,
                               5, groups=model_count)

        # conv2: present for depth >= 2
        if depth >= 2:
            self.conv2 = nn.Conv2d(int(6 * width_factor) * model_count,
                                   int(16 * width_factor) * model_count,
                                   5, groups=model_count)

        # Intermediate FC layers depend on depth
        if depth == 4:
            # 2c-3f: conv2 → fc1(120w) → fc2(feature_dim) → fc3(out)
            self.fc1 = nn.Conv2d(_after_2conv_flat * model_count,
                                 int(120 * width_factor) * model_count, 1, groups=model_count)
            self.fc2 = nn.Conv2d(int(120 * width_factor) * model_count,
                                 int(self.feature_dim) * model_count, 1, groups=model_count)
            _fc3_in = int(self.feature_dim)
        elif depth == 3:
            # 2c-2f: conv2 → fc2(feature_dim) → fc3(out)  [fc1 skipped]
            self.fc2 = nn.Conv2d(_after_2conv_flat * model_count,
                                 int(self.feature_dim) * model_count, 1, groups=model_count)
            _fc3_in = int(self.feature_dim)
        elif depth == 2:
            # 2c-1f: conv2 → fc3(out)  [fc1, fc2 skipped]
            _fc3_in = _after_2conv_flat
        else:  # depth == 1
            # 1c-1f: conv1 → fc3(out)  [conv2, fc1, fc2 skipped]
            _fc3_in = _after_1conv_flat

        self.fc3 = nn.Conv2d(_fc3_in * model_count, output_dim * model_count, 1, groups=model_count)
    ...
    def forward(self, x):
        # the forward takes in x shaped [# of examples, C_in, H, W]
        # outputs [# of examples, model_count, logit_count]
    ...
    def _all_layers(self):
        """Return existing conv/fc layers in forward order."""
        layers = [self.conv1]
        if self.depth >= 2:
            layers.append(self.conv2)
        if self.depth == 4:
            layers.extend([self.fc1, self.fc2])
        elif self.depth == 3:
    ...
    def get_feature(self, x, cat_one=False):
        ...
        if self.depth == 4:
            ...
            out = F.relu(self.fc2(out))
        feat_dim = out.shape[1]  # infer feature dim for any depth
        out = out.view(out.size(0), self.model_count, feat_dim)
        if cat_one:
    ...
    @torch.no_grad()
    def reinitialize_small(self):
        """Uniform [-0.2, 0.2] initialization for all parameters."""
        for para in self.parameters():
            torch.nn.init.uniform_(para.data, a=-0.2, b=0.2)

    @torch.no_grad()
    def reinitialize_kaiming_uniform(self):
        """Kaiming uniform initialization per layer (for ReLU activations)."""
        import math
    ...
    @torch.no_grad()
    def reinitialize_kaiming_normal(self):
        """Kaiming normal initialization per layer (for ReLU activations)."""
        import math
        for layer in self._all_layers:
            nn.init.kaiming_normal_(layer.weight, a=0, nonlinearity='relu')
            if layer.bias is not None:
                fan_in, _ = nn.init._calculate_fan_in_and_fan_out(layer.weight)
                bound = 1.0 / math.sqrt(fan_in) if fan_in > 0 else 0
                nn.init.uniform_(layer.bias, -bound, bound)
    ...
    def compute_lipschitz_constant(self, ref_data, batch_size=4):
        """Compute max_{z in ref_data} ||J_f(z)||_F per model using autograd.
        Exploits grouped-conv structure: input is repeated M times so each
        group of C_in channels feeds exactly one model, making per-model
        Jacobians extractable from a single backward pass per output class.
        Returns shape (model_count,)."""
        device = next(self.parameters()).device
        max_lip = torch.zeros(self.model_count, device=device)
        C_in = 3 if self.dataset == 'cifar10' else 1

        for st in range(0, len(ref_data), batch_size):
            x_batch = ref_data[st:st + batch_size].to(device)
            B = x_batch.shape[0]

            with torch.enable_grad():
                # Repeat input so each model gets its own C_in channel slice.
                # requires_grad on x_rep lets us extract per-model Jacobians.
                x_rep = x_batch.detach().repeat(1, self.model_count, 1, 1).requires_grad_(True)

                # Depth-aware forward (mirrors self.forward but without the initial repeat)
                out = F.relu(self.conv1(x_rep))
                out = F.max_pool2d(out, 2)
                if self.depth >= 2:
                    out = F.relu(self.conv2(out))
                    out = F.max_pool2d(out, 2)
                ...
                if self.depth == 4:
                    ...
                    out = F.relu(self.fc2(out))
                out = self.fc3(out)
                out = out.view(B, self.model_count, self.output_dim)

                # ||J_f||_F^2 = sum_c ||d(out[:,:,c])/d(x_rep)||^2
                lip_sq = torch.zeros(B, self.model_count, device=device)
                for c in range(self.output_dim):
                    grad = torch.autograd.grad(
                        out[:, :, c].sum(), x_rep,
                        retain_graph=(c < self.output_dim - 1)
                    )[0]  # [B, M*C_in, H, W]
                    # Reshape to isolate per-model gradient blocks
                    grad_per_model = grad.view(B, self.model_count, C_in, *x_batch.shape[2:])
                    lip_sq += grad_per_model.reshape(B, self.model_count, -1).pow(2).sum(dim=2)

                lip = lip_sq.detach().sqrt()  # [B, M]

            max_lip = torch.max(max_lip, lip.max(dim=0).values)
    ...
    @torch.no_grad()
    def forward_normalize_frobenius(self, x):
        """Original per-layer Frobenius-norm-based normalization."""
        x = self.forward(x)
        cum_norm = 1
        for layer in self._all_layers:
            cur_weight = layer.weight
...
class LinearModels(nn.Module):
    ...
    def compute_lipschitz_constant(self, ref_data=None):
        """For a linear model f(x)=Wx+b, J_f = W (constant across x).
        ||J_f||_F = ||W||_F, identical to Frobenius normalization.
        Returns shape (model_count,)."""
        return self.weight.norm(dim=(2, 3)).squeeze(0)  # [model_count]

    def forward_normalize_frobenius(self, x):
        """Normalize by Frobenius norm of weight matrix."""
        cur_norm = self.weight.norm(dim=(2, 3), keepdim=True).squeeze(3)
        return self.forward(x) / cur_norm

    def forward_normalize_lipschitz(self, x):
        """For linear models, Lipschitz constant == ||W||_F (data-independent),
        so this is equivalent to Frobenius normalization."""
        return self.forward_normalize_frobenius(x)

    def forward_normalize(self, x):
        if getattr(self, '_norm_mode', 'frobenius') == 'lipschitz':
            return self.forward_normalize_lipschitz(x)
        return self.forward_normalize_frobenius(x)
