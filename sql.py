...
@param('training_seed')
@param('data_seed')
def get_next_config(db_path, loss_bins, num_samples, excluded_cells=None, training_seed=None, data_seed=None):
    ...
    min_cnt = float('inf')
    next_loss_bin = None
    next_num_sample = None
    for loss_bin in loss_bins:
        for num_sample in num_samples:
            if (num_sample, loss_bin[0], loss_bin[1]) in excluded_cells:
                continue
    ...
    if next_loss_bin is None:
        con.close()
        raise RuntimeError("All cells are excluded; nothing left to train.")
