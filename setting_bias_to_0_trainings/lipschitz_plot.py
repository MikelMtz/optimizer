...
1. **Heatmap grid** (saved to ``--save_path``):
   ...
   Columns: one per selected hidden-unit width (``--sweep width``, default)
            or one per selected depth  (``--sweep depth``).
...
2. **Scatter overlay** (saved to ``--scatter_save_path``):
   ...
   selected sweep values, colour-coded.

Usage — width sweep (run from the check_bias_0/ directory):

    python lipschitz_plot.py \\
...
Usage — depth sweep:
    ...
    python lipschitz_plot.py \\
        --output_base output/kink_depth_sweep \\
        --sweep depth \\
        --depths 1 2 3 4 5 6 \\
        --init kaiming_uniform \\
        --dataset kink \\
        --save_path plots/lipschitz_depth_heatmap.pdf \\
        --scatter_save_path plots/lipschitz_depth_scatter.pdf
"""
import os
import sys
import glob
...
from datasets import Kink, MNIST

# ── Constants ─────────────────────────────────────────────────────────────────
ALL_WIDTHS = [2, 4, 8, 16, 32, 64, 128, 256, 512, 768, 1024]
ALL_DEPTHS = [1, 2, 3, 4, 5, 6]
...
def parse_args():
    ...
    p.add_argument("--sweep", default="width", choices=["width", "depth"],
                   help="Sweep over hidden-unit widths or network depths")
    p.add_argument("--widths", type=int, nargs="+", default=[4, 16, 64, 256],
                   help="Hidden-unit widths to plot as columns (width sweep)")
    p.add_argument("--depths", type=int, nargs="+", default=[1, 2, 3, 4, 5, 6],
                   help="Network depths to plot as columns (depth sweep)")
    p.add_argument("--init", default="kaiming_uniform", choices=INITS,
                   help="Weight initialisation to display")
    p.add_argument("--dataset", default="kink", choices=["kink", "mnist"],
    ...
    # MNIST-specific args
    p.add_argument("--mnist_classes", default="0,7",
                   help="Comma-separated MNIST class indices (e.g. '0,7')")
    p.add_argument("--mnist_train_count", type=int, default=16,
                   help="Number of MNIST training samples")
    p.add_argument("--mnist_data_seed", type=int, default=0,
                   help="Seed for MNIST data loading")
...
def _sweep_values(args):
    """Return the list of sweep values (widths or depths) for columns."""
    return args.depths if args.sweep == "depth" else args.widths


def _make_tag(args, opt_key, val):
    """Build the experiment directory tag for a given sweep value."""
    if args.sweep == "depth":
        return f"{opt_key.lower()}_{args.init}_d{val}"
    return f"{opt_key.lower()}_{args.init}_u{val}"


def _col_label(args, val):
    """Return the column header label for a sweep value."""
    if args.sweep == "depth":
        return f"Depth {val}"
    return f"Width {val}"
...
def collect_all_metrics(args, device, train_data, train_labels,
    ...
    """Return a dict  ``metrics[(row_i, sweep_val)] = (tl_np, ta_np)``."""
    metrics = {}  # (row_i, sweep_val) → (tl_np, ta_np)
    sweep_vals = _sweep_values(args)

    for col_i, val in enumerate(sweep_vals):
        for row_i, (opt_key, norm_mode, row_label, _cmap) in enumerate(ROWS):
            tag = _make_tag(args, opt_key, val)
            short_label = row_label.split("\n")[0]
            sweep_name = "depth" if args.sweep == "depth" else "width"

            model_dir = os.path.join(args.output_base, tag, "models")
            print(f"[{short_label} / {norm_mode} / {sweep_name}={val}] "
            ...
            metrics[(row_i, val)] = (tl_cat.numpy(), ta_cat.numpy())
...
def plot_heatmap(args, metrics):
    """Original 2-D density heatmap grid (rows × sweep-value columns)."""
    n_rows = len(ROWS)
    sweep_vals = _sweep_values(args)
    n_cols = len(sweep_vals)
    ...
    for col_i, val in enumerate(sweep_vals):
        for row_i, (opt_key, norm_mode, row_label, cmap_name) in enumerate(ROWS):
            ...
            if (row_i, val) not in metrics:
                ax.text(0.5, 0.5, "No data", ha="center", va="center",
                        transform=ax.transAxes, fontsize=10, color="gray")
                _decorate_ax(ax, row_i, col_i, n_rows,
                             _col_label(args, val), row_label)
                continue

            tl_np, ta_np = metrics[(row_i, val)]
            total = len(tl_np)
            mean_acc = ta_np.mean()
    ...
    sweep_label = "Depth sweep" if args.sweep == "depth" else "Width sweep"
    dataset_label = "MNIST" if args.dataset == "mnist" else "Kink"
    fig.suptitle(
        f"Test Accuracy vs. Normalised Train Loss \u2014 {dataset_label} dataset {sweep_label} \u2014 Init: {INIT_DISPLAY.get(args.init, args.init)}",
        fontsize=13, y=1.02,
    )
...
def plot_scatter_overlay(args, metrics):
    ...
    lines from every requested sweep value, colour-coded."""
    n_rows = len(ROWS)
    sweep_vals = _sweep_values(args)

    # Build a colour map for the sweep values
    cmap = plt.get_cmap("tab10")
    val_colors = {v: cmap(i % 10) for i, v in enumerate(sweep_vals)}
    ...
    for row_i, (opt_key, norm_mode, row_label, _cmap_name) in enumerate(ROWS):
        ...
        if has_data:
            ax.legend(fontsize=7, loc="lower left", framealpha=0.85,
                      ncol=min(len(sweep_vals), 4))

    sweep_label = "Depth sweep" if args.sweep == "depth" else "Width sweep"
    dataset_label = "MNIST" if args.dataset == "mnist" else "Kink"
    fig.suptitle(
        f"Mean Test Accuracy per Loss Bin for {dataset_label} dataset - {sweep_label} \u2014 Init: {INIT_DISPLAY.get(args.init, args.init)}",
        fontsize=13, y=1.02,
    )
...
# ── Main ──────────────────────────────────────────────────────────────────────
def _load_datasets(args, device):
    """Load train/test tensors for the chosen dataset."""
    if args.dataset == "mnist":
        classes = [int(c) for c in args.mnist_classes.split(",")]
        num_classes = len(classes)
        ds = MNIST(
            batch_size=args.mnist_train_count,
            threads=0,
            aug="none",
            train_count=args.mnist_train_count,
            num_classes=num_classes,
            seed=args.mnist_data_seed,
            classes=classes,
        )
        train_data, train_labels = next(iter(ds.train))
        train_data = train_data.view(train_data.size(0), -1).float().to(device)
        train_labels = train_labels.long().to(device)
        test_data = ds.test_all_data.view(ds.test_all_data.size(0), -1).float().to(device)
        test_labels = ds.test_all_labels.long().to(device)
    else:
...
def main():
    ...
    # ---- Evaluation datasets ------------------------------------------------
    train_data, train_labels, test_data, test_labels = _load_datasets(args, device)
...
def _decorate_ax(ax, row_i, col_i, n_rows, col_title, row_label):
    """Apply shared axis labels and titles to *ax*.

    *col_title* is a string like "Width 64" or "Depth 3".
    """
    if row_i == n_rows - 1:
        ax.set_xlabel("Train loss (normalized)", fontsize=9)
    if col_i == 0:
