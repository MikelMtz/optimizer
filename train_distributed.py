        ...
        if epoch == int(epochs * EARLY_STOP_WARMUP_FRAC):
            with torch.no_grad():
                es_loss, es_acc_check = calculate_loss_acc(
                    train_data, train_labels, model.forward_normalize, loss_func)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Cap model count to prevent OOM for small num_samples
        MAX_MODEL_COUNT = 30000
        if cur_model_count > MAX_MODEL_COUNT:
            print(f"Capping model_count from {cur_model_count} to {MAX_MODEL_COUNT}")
            cur_model_count = MAX_MODEL_COUNT
